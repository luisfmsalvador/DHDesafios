...
class CardapioAdapter(val dataSet: List<Cardapio>, private val listener: (Cardapio) -> Unit) :
    RecyclerView.Adapter<CardapioAdapterHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): CardapioAdapterHolder {
        val inflador =
            LayoutInflater.from(parent.context).inflate(R.layout.lista_pratos, parent, false)

        return CardapioAdapterHolder(
            inflador
        )
    }
